...
vartools=['MuTect2 | ', 'VarScan2 | ', 'VarDict | ', 'MIPS_VarCaller | ']
...
for row in x:
    ...
    readcounts_split = readcounts.split(',')
    ref_count = int(readcounts_split[0]) + int(readcounts_split[1])
    alt_count = int(readcounts_split[2]) + int(readcounts_split[3])    
    ...
    vaf=(float(info[-1].split('=')[1])) * 100
    data['VAF%'].append(round (vaf, 2 ))
...
horizontal_stack['AAChange.refGene']=horizontal_stack['AAChange.refGene'].astype(str).replace(',' , ';')
